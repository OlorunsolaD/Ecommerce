...
class MyTokenObtainPairSerializer(TokenObtainPairSerializer):
    def validate(self, attrs):
        data = super().validate(attrs)  # Step 1: Validate user credentials and get token data
        serializer = UserSerializerWithToken(self.user).data  # Step 2: Serialize the authenticated user
        for k, v in serializer.items():  # Step 3: Merge user data into the token data
            data[k] = v
